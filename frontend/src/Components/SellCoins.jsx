...
const SellCoins = memo(({
  ...
  const coinUsdPrice = useRef(() => currencyConverter(data?.preMarketPrice ? data?.preMarketPrice : data?.regularMarketPrice, currencyConversion.rates[data?.currency]));
  ...
  useEffect(() => {
    ...
    async function coinAmount() {
      const res = await fetch(`https://api-6tyd64odzq-uc.a.run.app/api/user/getPurchasedStock?id=${currentUser?.uid}`);
      if (!res.ok) {
  ...
  async function onPlaceOrder() {
    try {
      ...
      if (coinValue.current.value > availabeCoinAmt) {
      ...
      const sellStock = await fetch("https://api-6tyd64odzq-uc.a.run.app/api/user/sellStock", {
        ...
        body: JSON.stringify({
          id: currentUser.uid,
          stockAmount: parseInt(coinValue.current.value),
          stockPrice: Math.round(parseFloat(coinUsdPrice.current.value) * 1e3) / 1e3,
          symbol: data?.symbol,
        ...
        <div className="relative  rounded-lg shadow bg-gray-700">
          ...
          <div className="px-6 py-3 md:p-6">
            ...
            <div className="relative py-4">
              ...
              <input type="number" id="coinValue" name="coinValue" min="0" ref={coinUsdPrice} className=" border   text-sm rounded-lg  block w-full pl-10 p-2.5  bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" />
            </div>
            ...
            <div className="relative py-4">
              ...
              <input type="number" min="0" id="coinUsdValue" name="coinUsdValue" value={coinUsdPrice.current.value} className=" border   text-sm rounded-lg block w-full pl-10 p-2.5  bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" />
            </div>
          </div>
          {/* Modal footer  */}
