...
const CoinsTable = memo(() => {
  ...
  const handlePagination = data => {
    page.current = Number(data.selected + 1);
  };
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [page.current]);
  ...
  return <div className="z-10">
      ...
      {fetchGlobalCryptoSuccess && <div className="no-scrollbar flex  p-4 space-x-4 rounded-box w-screen overflow-scroll max-w-md md:max-w-full lg:flex-wrap">
          <div className="">
            ...
            <div className=" bg-gradient-to-tr from-gray-900 to-gray-700   overflow-hidden shadow rounded-lg w-60 md:w-72 relative">
              ...
              <div className="px-4 py-5 sm:p-6">
                <dl>
                  ...
                  <dd className={`font-text mt-1 text-3xl leading-9 font-semibold ${globalCryptoData.data.market_cap_change_percentage_24h_usd >= 0 ? "text-green-400" : "text-red-400"} `}>
      ...
      {fetchGlobalCryptoSuccess && <Link to="/app/market/globalStats" className="text-md  font-semibold text-green-400 px-4 underline">
      ...
      <ul className="md:px-4 flex flex-col space-y-1 pb-12 font-text text-white">
        ...
        {/* coin prices */}
        {isSuccess && data?.map((coins, index) => <li key={index} onClick={() => navigate(`/app/coin/${coins.id}`)} className="grid grid-cols-2 md:grid-cols-4 text-gray-500 py-2 px-1md:px-5 hover:bg-gray-900 rounded-lg cursor-pointer border-b-2 border-gray-800 ">
      ...
      {/* pagination */}
      <ReactPaginate previousLabel={"<"} nextLabel={">"} breakLabel={"..."} pageCount={52} marginPagesDisplayed={2} pageRangeDisplayed={1} onPageChange={handlePagination} containerClassName={`flex justify-center space-x-2 text-xs font-medium text-white`} pageClassName={`inline-flex items-center justify-center w-8 h-8 border text-white border-gray-100 rounded-full`} pageLinkClassName={`block w-8 h-8 leading-8 text-center text-white  border-green-600 rounded-full`} previousLinkClassName={`block w-8 h-8 leading-8 text-center text-white  bg-green-600 border-green-600 rounded-full`} nextLinkClassName={`block w-8 h-8 leading-8 text-center text-white  bg-green-600 border-green-600 rounded-full`} />
    </div>;
});
